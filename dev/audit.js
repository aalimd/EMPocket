...
(async function () {
  ...
  const source = await (await fetch('../assets/app.js?v=20260910-explorer-r2')).text();
  ...
  const parsed = new DOMParser().parseFromString(await (await fetch('../index.html?audit=20260910-explorer-r2')).text(), 'text/html');
    ...
    for (const width of [320, 390, 768, 1280]) {
      ...
      for (const id of ['home', 'ecg', 'ecg-explorer', 'study', 'shift', ...window.CP_DATA.map(cp => cp.id)]) {
        ...
        check('Route ' + id + ' at ' + width + 'px', () => {
          ...
          assert(doc.documentElement.clientWidth === width, 'Wrong fixture viewport');
